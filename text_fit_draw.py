...
import re
import sys
# ==== 新增：emoji 相关占位，避免未定义引用 ====
TWEMOJI_BASE = "https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/"  # 仅占位

def emoji_cluster_to_filename(cluster: str) -> str:
    return f"{cluster}.png"

def download_emoji_png(url: str, path: str, timeout: float, retries: int):
    return None  # 占位不下载

def emoji_advance_px(font: ImageFont.ImageFont) -> int:
